...
#-------------------------------------------------------------------------------
# Name:        PID
# Purpose:   定水深ブイの制御をPIDで行い、その制御パラメータを求める。
# memo: なんか作りかけのまま・・・
# Author:      morishita
#
# Created:     01/02/2014
...
class pid_operater:
    """ PIDに基づく制御オペレータ
    他の制御則オペレータとの互換性に気をつけて関数の設計を行う必要がある。
    """
    def __init__(self, parameter):
        _pid_param, integral_term = parameter
        self._param = _pid_param
        self._integral_term = integral_term     # unit is [s]

    def get_control_value(self, parameter):
        time_list, speed, current_position, target_position = parameter # それぞれが時刻と紐付いたリスト（履歴）って事にならんかな？
        
        current_velocity = speed[-1]
        current_diff = target_position[-1] - current_position[-1]
        current_diff_integral = 0.0

        for i in range(len(time_list)):
            t = time_list[i]
            if t > time_list[-1] - self._integral_term: # 時刻の計算って銅やった？
                current_diff_integral += target_position[i] - current_position[i]

        ans = self._param.k_p * current_diff + self._param.k_i * current_diff_integral + self._param.k_d * current_velocity
        return ans
...
class sea_filed:
    ...
    def get_pressure(self, x, y, z, t):
        return None


class floater:
    def __init__(self, operater, time_step = 0.01):
        self._pos = position()
        self._speed = velocity()
        self._accel = acceleration()
        self._target_depth = 0.0
        self._operater = operater
        self._time_step = time_step
        self._time = 0.0
        self._history_time = [self._time]
        self._history_pos = [self._pos]
        self._history_speed = [self._speed]
